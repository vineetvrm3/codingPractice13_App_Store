...
class AppStore extends Component {
  ...
  getSearchInput = event => {
    this.setState({searchInput: event.target.value.replace(' ', '')})
  }

  render() {
    const {activeTabId, searchInput} = this.state

    const activeTabList = appsList.filter(
      each =>
        activeTabId === each.category &&
        each.appName.toLowerCase().includes(searchInput.replace(' ', '')),
    )
    return (
